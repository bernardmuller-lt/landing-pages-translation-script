...
function showUsage() {
  ...
  console.log('\nSupported locales:');
  console.log('  de_de (German), es_419 (Spanish, Latin America), ko_kr (Korean)');
  console.log('  pt_br (Portuguese, Brazil), fr_fr (French), nl_nl (Dutch)');
  console.log('  it_it (Italian), ja_jp (Japanese), pl_pl (Polish)');
  console.log('  da_dk (Danish), no_no (Norwegian), zh_cn (Chinese, Simplified)');
  console.log('\nModel:');
  console.log('  - Optional: can be passed as third argument or set via LLM_MODEL env var');
...
async function main() {
  ...
  // Get slug and locale from command-line arguments
  ...
  // Get model from CLI argument or environment variable
  const model = modelArg || process.env.LLM_MODEL;
  if (!model) {
    console.error('\n❌ Error: Model not specified\n');
    console.error('Please either:');
    console.error('  1. Pass model as third argument: npm run translate -- home de_de gpt-4o-mini');
  ...
  const outputPath = join(config.translationsOutputDir, `${slug}-${targetLocale}.json`);

  // Check if input file exists
  if (!existsSync(inputPath)) {
    console.error(`\n❌ Error: Input file not found: ${inputPath}`);
    console.error('Run the parse command first: npm run parse -- ' + slug + '\n');
    process.exit(1);
  }
  ...
  try {
    ...
    const translatedStrings = await translateToLocale(englishTranslations, targetLocale, model);
    ...
    console.error('\n❌ Fatal error:', error);
    console.error('\nTroubleshooting:');
    console.error('  1. Check LLM_BASE_URL in .env file (e.g., https://api.openai.com/v1)');
    ...
    console.error('  5. For local models (Ollama): ensure service is running\n');
    process.exit(1);
  }
}
